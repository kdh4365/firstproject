...
class CommentRepositoryTest {
    ...
    void findByNickname() {
        ...
        /* Case 3: null의 모든 댓글 조회 */
        {
            ...
            List <Comment> expected = Arrays.asList();
            //검증
            assertEquals(expected, comments, "닉네임을 작성해 주세요.");
        }
    ...
    void findByNickname1() {
        ...
        /* Case 5: "i"가 들어가있는 모든 댓글 조회 */
        {
            ...
            // 실제 수행
            List<Comment> comments = commentRepository.findByNickname(nickname1);
            List<Comment> comments2 = commentRepository.findByNickname(nickname2);
            comments.addAll(comments2);
            //예상하기
            Comment a = new Comment(2L, new Article(4L, "당신의 인생 영화는?", "댓글 ㄱ"), "Kim", "아이 엠 셈");
            ...
            Comment d = new Comment(3L, new Article(4L, "당신의 인생 영화는?", "댓글 ㄱ"), "Choi", "쇼생크의 탈출");
            Comment e = new Comment(6L, new Article(5L, "당신의 소울 푸드는?", "댓글 ㄱㄱ"), "Choi", "초밥");
            Comment f = new Comment(9L, new Article(6L, "당신의 취미는?", "댓글 ㄱㄱㄱ"), "Choi", "독서");
            List <Comment> expected = Arrays.asList(a, b, c, d, e, f);

            //검증
            assertEquals(expected.toString(), comments.toString(), "'i'가 들어가있는 닉네임의 모든 댓글 조회");
        }
    }
}